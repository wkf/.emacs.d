...
(dolist (file '("bootstrap.el" "private.el" "custom.el"))
...
(use-package base16-theme
  ...
  (defvar user/colors
    `(
      :black ,(plist-get base16-material-darker-theme-colors :base00)
      :gray0 "gray15"
      :gray1 "gray17"
      :gray2 "gray23"
      :gray3 ,(plist-get base16-material-darker-theme-colors :base01)
      :gray4 ,(plist-get base16-material-darker-theme-colors :base02)
      :gray5 ,(plist-get base16-material-darker-theme-colors :base03)
      :blueish ,(plist-get base16-material-darker-theme-colors :base04)
      :cyanish ,(plist-get base16-material-darker-theme-colors :base05)
      ;; :base05 and :base06 are the same
      :white ,(plist-get base16-material-darker-theme-colors :base07)
      :red ,(plist-get base16-material-darker-theme-colors :base08)
      :orange ,(plist-get base16-material-darker-theme-colors :base09)
      :yellow ,(plist-get base16-material-darker-theme-colors :base0A)
      :green ,(plist-get base16-material-darker-theme-colors :base0B)
      :cyan ,(plist-get base16-material-darker-theme-colors :base0C)
      :blue ,(plist-get base16-material-darker-theme-colors :base0D)
      :magenta ,(plist-get base16-material-darker-theme-colors :base0E)
      :pink ,(plist-get base16-material-darker-theme-colors :base0F)))

  (set-face-attribute 'fringe nil
   :foreground (plist-get user/colors :gray5)
   :background (plist-get user/colors :black))

  (set-face-attribute 'font-lock-comment-face nil :slant 'italic)
  (set-face-attribute 'lazy-highlight nil :weight 'bold)
  (set-face-attribute 'isearch nil :weight 'bold)
  (set-face-attribute 'isearch-fail nil :weight 'bold)
...
(use-package display-line-numbers
  ...
  :custom-face
  (line-number
   ((t (:foreground ,(plist-get user/colors :gray5)
        :background ,(plist-get user/colors :black)))))
...
;;   :straight nil
;;   :load-path "user"
;;   :after (powerline spinner)
;;   :init
;;   (setq user-mode-line/emacs-state-color (plist-get user/colors :yellow)
;;         user-mode-line/insert-state-color (plist-get user/colors :green)
;;         user-mode-line/motion-state-color (plist-get user/colors :magenta)
;;         user-mode-line/normal-state-color (plist-get user/colors :blue)
;;         user-mode-line/replace-state-color (plist-get user/colors :pink)
;;         user-mode-line/visual-state-color (plist-get user/colors :orange)
;;         user-mode-line/special-state-color (plist-get user/colors :cyan)
;;         user-mode-line/unknown-state-color (plist-get user/colors :red))
;;   :config
...
;;   (defun user/lispy-ace-symbol (arg)
;;     "A version of lispy-ace-symbol that behaves more like ace-lispy-paren."
;;     (interactive "p")
;;     (setq arg (or arg 1))
;;     (lispy--remember)
;;     (deactivate-mark)
...
;;   (defun user/pprint-eval-defun-dwim ()
;;     (interactive)
;;     (cond ((eq major-mode 'emacs-lisp-mode)
;;            (call-interactively 'eval-defun))
;;           ((eq major-mode 'clojure-mode)
...
;;     (interactive)
;;     (cond ((lispyville--in-string-p))
;;           ((and (lispyville--after-left-p) (looking-at " "))
;;            (delete-char 1))
;;           ((and (lispyville--at-right-p) (save-excursion (backward-char) (looking-at " ")))
;;            (delete-char -1)))
;;     (evil-normal-state))

;;   (defun user/lispy-forward-open (arg)
;;     "A version of lispy-flow that only works on open parens."
;;     (interactive "p")
;;     (lispy--remember)
;;     (let ((pt (point))
;;           (r (lispy--re-search-in-code lispy-left 'forward arg)))
;;       (if r
;;           r
;;         (goto-char pt)
;;         nil)))

;;   (defun user/lispy-backward-open (arg)
;;     (interactive "p")
...
;;                    (error "Unexpected"))
;;                  (setq end (point))))))
;;       (let* ((lispy-avy-keys (delete ?p lispy-avy-keys))
...
;;               (t
;;                ;; TODO: don't skip ahead if in a quoted list
;;                (when (looking-at "(")
;;                  (forward-char 1)
;;                  (unless (looking-at "(")
;;                    (ignore-errors
...
;;    :around (defun user/around-lispyville-atom-end (f arg)
...
;;            user/lispy-teleport-sexp
;;            user/eval-sexp-dwim
;;            user/pprint-eval-sexp-dwim
;;            user/eval-defun-dwim
...
;;    "}" 'lispy-right-nostring
;;    ")" 'lispy-right-nostring
;;    "C-9" 'user/lispy-insert-at-beginning-of-list
;;    "C-0" 'user/lispy-insert-at-end-of-list)
;;   ('insert
;;    'lispyville-mode-map
;;    "<escape>" 'user/lispy-escape)
;;   ('(normal visual)
;;    'lispyville-mode-map
;;    "(" 'lispyville-backward-up-list
;;    ")" 'lispyville-up-list
...
;;    "SPC l" 'lispy-ace-paren
;;    "SPC a" 'user/lispy-ace-symbol
;;    "g/" 'lispy-occur
;;    "g+" 'lispy-widen
;;    "g-" 'lispy-narrow
